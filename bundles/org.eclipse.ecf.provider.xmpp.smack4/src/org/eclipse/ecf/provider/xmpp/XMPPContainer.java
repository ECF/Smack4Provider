/*******************************************************************************
 * Copyright (c) 2004, 2015 Composent, Inc. and others. All rights reserved. This
...
public class XMPPContainer extends ClientSOContainer implements IPresenceService {
	...
	private static Set<String> googleNames = new HashSet<String>();
	...
	public void connect(ID remote, IConnectContext joinContext) throws ContainerConnectException {
		try {
			...
			throw new ContainerConnectException(
					NLS.bind(Messages.XMPPContainer_EXCEPTION_ADDING_SHARED_OBJECT, presenceHelperID), e1);
		}
	}
	...
	protected void handleXMPPMessage(Stanza aPacket) throws IOException {
	...
	protected boolean handleAsExtension(Stanza packet) {
	...
	protected void processAsynch(AsynchEvent e) {
		try {
			if (e instanceof ECFConnectionPacketEvent) {
				...
				handleXMPPMessage((Stanza) e.getData());
				return;

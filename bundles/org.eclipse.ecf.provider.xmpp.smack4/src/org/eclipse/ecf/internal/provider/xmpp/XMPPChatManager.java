...
public class XMPPChatManager implements IChatManager {
	...
	private final List<IIMMessageListener> messageListeners = new ArrayList<IIMMessageListener>();
	...
	private final IChatMessageSender chatMessageSender = new IChatMessageSender() {
		 ...
		 * java.lang.String)
		 */
		public void sendChatMessage(ID toID, ID threadID, org.eclipse.ecf.presence.im.IChatMessage.Type type,
				String subject, String body, Map properties) throws ECFException {
			if (toID == null)
				throw new ECFException("receiver cannot be null");
			try {
				presenceHelper.getConnectionOrThrowIfNull().sendMessage(toID, threadID,
						XMPPChatManager.this.createMessageType(type), subject, body, properties);
			} catch (final Exception e) {
				throw new ECFException("sendMessage exception", e);
			}
	...
	protected ITypingMessageSender typingMessageSender = new ITypingMessageSender() {
		...
		public void sendTypingMessage(ID toID, boolean isTyping, String body) throws ECFException {
			...
			try {
				...
				throw new ECFException("sendTypingMessage exception", e);
			}
		}
	...
	protected IHistoryManager historyManager = new IHistoryManager() {
		 ...
		 * (non-Javadoc)
		 * 
		 * @see org.eclipse.ecf.presence.im.IChatHistoryManager#getHistory(org.
		 * eclipse .ecf.core.identity.ID, java.util.Map)
		 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.eclipse.ecf.presence.im.IChatManager#addChatMessageListener(org.
	...
	private void fireMessageEvent(final IIMMessageEvent event) {
		List toNotify = null;
		synchronized (messageListeners) {
			toNotify = new ArrayList<IIMMessageListener>(messageListeners);
		}
	...
	protected void fireChatMessage(ID fromID, ID threadID, Type type, String subject, String body, Map properties) {
		fireMessageEvent(new ChatMessageEvent(fromID,
				new ChatMessage(fromID, threadID, createMessageType(type), subject, body, properties)));
	}
	...
	protected void fireXHTMLChatMessage(ID fromID, ID threadID, Type type, String subject, String body, Map properties,
			List xhtmlbodylist) {
		fireMessageEvent(new XHTMLChatMessageEvent(fromID, new XHTMLChatMessage(fromID, threadID,
				createMessageType(type), subject, body, properties, xhtmlbodylist)));

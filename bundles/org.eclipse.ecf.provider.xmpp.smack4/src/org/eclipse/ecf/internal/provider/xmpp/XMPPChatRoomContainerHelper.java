...
public class XMPPChatRoomContainerHelper implements ISharedObject {
	...
	private final List<IChatRoomParticipantListener> participantListeners = new ArrayList<IChatRoomParticipantListener>();
	private ID roomID = null;

	private final List<ID> chatRoomContainerParticipants = Collections.synchronizedList(new ArrayList<ID>());
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.eclipse.ecf.core.ISharedObject#init(org.eclipse.ecf.core.
	 * ISharedObjectConfig)
	 */
	...
	protected void fireMessageListeners(ID from, String body) {
		List<IIMMessageListener> toNotify = null;
		synchronized (messageListeners) {
			toNotify = new ArrayList<IIMMessageListener>(messageListeners);
		}
		for (final Iterator<IIMMessageListener> i = toNotify.iterator(); i.hasNext();) {
	...
	protected void handleMessageEvent(MessageEvent evt) {
		...
		if (msg != null)
			fireMessageListeners(createUserIDFromName(canonicalizeRoomFrom(msg.getFrom().toString())), msg.getBody());
	}
	...
	protected IPresence createIPresence(Presence xmppPresence) {
		...
		final IPresence newPresence = new org.eclipse.ecf.presence.Presence(createIPresenceType(xmppPresence), status,
	...
	protected void handlePresenceEvent(PresenceEvent evt) {
		...
		final String from = canonicalizeRoomFrom(xmppPresence.getFrom().toString());
	...
	protected void fireParticipant(ID fromID, IPresence presence) {
		List<IChatRoomParticipantListener> toNotify = null;
		synchronized (participantListeners) {
			toNotify = new ArrayList<IChatRoomParticipantListener>(participantListeners);
		}
		for (final Iterator<IChatRoomParticipantListener> i = toNotify.iterator(); i.hasNext();) {
	 ...
	 * 
	 * @see
	 * org.eclipse.ecf.core.ISharedObject#handleEvents(org.eclipse.ecf.core.util
	 * .Event[])
	 */

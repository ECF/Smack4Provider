...
public class XMPPContainerPresenceHelper implements ISharedObject {
	...
	private final List<ISharedObjectMessageListener> sharedObjectMessageListeners = new ArrayList<ISharedObjectMessageListener>();
	...
	class PresenceRosterManager extends AbstractRosterManager {
		 ...
		 * (non-Javadoc)
		 * 
		 * @see org.eclipse.ecf.presence.roster.AbstractRosterManager#
		 * getPresenceSender ()
		 */
	...
	protected void handleSharedObjectMessageEvent(ISharedObjectMessageEvent event) {
		List<ISharedObjectMessageListener> toNotify = null;
		synchronized (sharedObjectMessageListeners) {
			toNotify = new ArrayList<ISharedObjectMessageListener>(sharedObjectMessageListeners);
		}
		for (final Iterator<ISharedObjectMessageListener> i = toNotify.iterator(); i.hasNext();) {
	...
	private void addUnique(Collection<IRosterItem> existingItems, IRosterItem newItem) {
		List<IRosterItem> toAdd = new ArrayList<IRosterItem>();
		if (existingItems.size() == 0)
			toAdd.add(newItem);
		else {
			boolean addNewItem = true;
			for (Iterator<IRosterItem> i = existingItems.iterator(); i.hasNext();) {
		...
		for (Iterator<IRosterItem> i = toAdd.iterator(); i.hasNext();)
			existingItems.add(i.next());
	}

	private void addUniqueToRoster(IRosterItem[] newItems) {
		@SuppressWarnings("unchecked")
		Collection<IRosterItem> existingItems = (Collection<IRosterItem>) roster.getItems();
	...
	protected IRosterEntry createRosterEntry(RosterEntry entry) {
		XMPPID xmppid = createIDFromName(entry.getUser().toString());
	...
	protected void handleIQEvent(IQEvent evt) {
		...
		if (iq instanceof RosterPacket) {
			...
			if (rosterPacket.getType() == IQ.Type.set || rosterPacket.getType() == IQ.Type.result) {
				for (final Iterator<RosterPacket.Item> i = rosterPacket.getRosterItems().iterator(); i.hasNext();) {
					final RosterPacket.Item item = (RosterPacket.Item) i.next();
					final RosterPacket.ItemType itemType = item.getItemType();
					XMPPID newID = createIDFromName(item.getUser().toString());
					boolean remove = false;
					if (itemType == RosterPacket.ItemType.none || itemType == RosterPacket.ItemType.remove) {
						removeItemFromRoster(roster.getItems(), createIDFromName(item.getUser().toString()));
						remove = true;
					} else {
						remove = false;
	...
	private void removeItemFromRoster(Collection<IRosterItem> rosterItems, XMPPID itemIDToRemove) {
		boolean removed = false;
		synchronized (rosterItems) {
			for (final Iterator<IRosterItem> i = rosterItems.iterator(); i.hasNext();) {
	...
	private boolean removeItemFromRosterGroup(org.eclipse.ecf.presence.roster.RosterGroup group,
			XMPPID itemIDToRemove) {
		@SuppressWarnings("unchecked")
		final Collection<IRosterEntry> groupEntries = (Collection<IRosterEntry>) group.getEntries();
		synchronized (groupEntries) {
			for (final Iterator<IRosterEntry> i = groupEntries.iterator(); i.hasNext();) {
	...
	protected void handleMessageEvent(MessageEvent evt) {
		...
		final String from = msg.getFrom().toString();
		...
		if (msg != null) {
			if (msg.getExtension("composing", //$NON-NLS-1$
				...
				final List<CharSequence> xhtmlbodies = evt.getXHTMLBodies();
				if (xhtmlbodies != null) {
					chatManager.fireXHTMLChatMessage(fromID, threadID, msg.getType(), subject, body,
							ECFConnection.getPropertiesFromPacket(msg), xhtmlbodies);
				} else if (body != null) {
					chatManager.fireChatMessage(fromID, threadID, msg.getType(), subject, body,
	...
	private void firePresenceListeners(ID fromID, IPresence presence) {
		...
		for (final Iterator<IPresenceListener> i = toNotify.iterator(); i.hasNext();) {
	...
	private void updatePresence(XMPPID fromID, IPresence newPresence) {
		@SuppressWarnings("unchecked")
		final Collection<IRosterItem> rosterItems = (Collection<IRosterItem>) roster.getItems();
		List<AdditionalClientRosterEntry> newEntrys = new ArrayList<AdditionalClientRosterEntry>();
		synchronized (rosterItems) {
			for (@SuppressWarnings("unchecked")
			final Iterator<IRosterItem> i = roster.getItems().iterator(); i.hasNext();) {
				final IRosterItem item = (IRosterItem) i.next();
				if (item instanceof IRosterGroup) {
					...
					AdditionalClientRosterEntry entry = updatePresenceForMatchingEntry(
							(org.eclipse.ecf.presence.roster.RosterEntry) item, fromID, newPresence);
					if (entry != null)
						newEntrys.add(entry);
				}
			}
		...
		if (entrys.length > 0) {
			for (int i = 0; i < entrys.length; i++) {
				if (entrys[i].add) {
					IRosterEntry entry = new org.eclipse.ecf.presence.roster.RosterEntry(entrys[i].parent,
							entrys[i].user, entrys[i].presence);
	...
	private AdditionalClientRosterEntry removeEntryFromRoster(XMPPID oldID,
			org.eclipse.ecf.presence.roster.RosterEntry entry, IPresence newPresence, IUser user) {
	...
	private AdditionalClientRosterEntry updatePresenceForMatchingEntry(
			org.eclipse.ecf.presence.roster.RosterEntry entry, XMPPID fromID, IPresence newPresence) {
		...
		if (oldID.equals(fromID)) {
			...
			if (oldID.getResourceName() == null) {
				...
				return new AdditionalClientRosterEntry(entry.getParent(), new User(fromID, user.getName()),
						newPresence);
			}
		}
		return null;
	...
	private AdditionalClientRosterEntry[] updatePresenceInGroup(IRosterGroup group, XMPPID fromID,
		...
		synchronized (groupEntries) {
			for (final Iterator i = group.getEntries().iterator(); i.hasNext();) {
				AdditionalClientRosterEntry newEntry = updatePresenceForMatchingEntry(
						(org.eclipse.ecf.presence.roster.RosterEntry) i.next(), fromID, newPresence);
				if (newEntry != null)
	...
	protected void handleRoster(Roster roster) {
		...
		final Iterator i = roster.getEntries().iterator(); i.hasNext();) {
	...
	private final Map<String, VCard> vcardCache = new WeakHashMap<String, VCard>();
	...
	class XMPPPresence extends org.eclipse.ecf.presence.Presence {
		...
		XMPPPresence(String fromID, Presence xmppPresence, IFuture future) {
			super(createIPresenceType(xmppPresence), xmppPresence.getStatus(), createIPresenceMode(xmppPresence),
					ECFConnection.getPropertiesFromPacket(xmppPresence), null);
			this.fromID = fromID;
		}
	...
	protected IPresence createIPresence(final Presence xmppPresence) {
		...
		return new XMPPPresence(xmppPresence.getFrom().toString(), xmppPresence, asyncResult);
	}

	protected Presence createPresence(IPresence ipresence) {
		final Presence newPresence = new Presence(createPresenceType(ipresence), ipresence.getStatus(), 0,
	...
	protected IRosterItem[] createRosterEntries(RosterEntry entry) {
		final XMPPID xmppid = createIDFromName(entry.getUser().toString());
		final String name = entry.getName();
		final User newUser = (name == null) ? new User(xmppid, xmppid.getUsername())
				: new User(xmppid, XMPPID.unfixEscapeInNode(name));
			...
			for (; grps.hasNext();) {
				...
				if (findRosterEntry(rosterGroup, user) == null) {
					...
					new org.eclipse.ecf.presence.roster.RosterEntry(rosterGroup, user,
							new org.eclipse.ecf.presence.Presence(IPresence.Type.UNAVAILABLE,
	...
	protected IRosterEntry createRosterEntry(ID userID, String name, Iterator grps) {
		...
		if (groups.size() == 0)
			return new org.eclipse.ecf.presence.roster.RosterEntry(roster, user, new org.eclipse.ecf.presence.Presence(
					IPresence.Type.UNAVAILABLE, IPresence.Type.UNAVAILABLE.toString(), IPresence.Mode.AWAY));
		else
	...
	private void createRosterEntries(IRosterItem parent, IUser user, List result) {
		...
		if (oldEntry == null) {
			final org.eclipse.ecf.presence.roster.RosterEntry newEntry = new org.eclipse.ecf.presence.roster.RosterEntry(
					parent, user, new org.eclipse.ecf.presence.Presence(IPresence.Type.UNAVAILABLE,
							IPresence.Type.UNAVAILABLE.toString(), IPresence.Mode.AWAY));
			result.add(newEntry);
		}
	}
	...
	protected Message.Type[] ALLOWED_MESSAGES = { Message.Type.chat, Message.Type.error, Message.Type.headline,
			Message.Type.normal };

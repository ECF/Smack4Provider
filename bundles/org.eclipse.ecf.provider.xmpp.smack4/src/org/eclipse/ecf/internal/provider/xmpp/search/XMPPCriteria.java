...
public class XMPPCriteria implements ICriteria {
	...
	/** criteria list */
	protected final List<ICriterion> criteria;

	/**
	 * Create a criteria with a sync list
	 */
	public XMPPCriteria() {
		criteria = Collections.synchronizedList(new ArrayList<ICriterion>());
	}
	 ...
	 * 
	 * @see
	 * org.eclipse.ecf.presence.search.ICriteria#add(org.eclipse.ecf.presence.
	 * search.ICriterion)
	 */
	...
	public String toString() {
		...
		Iterator<ICriterion> it = criteria.iterator();
		while (it.hasNext()) {
			sb.append("[");//$NON-NLS-1$
